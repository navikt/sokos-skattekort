...
class SftpConfig(
    private val sftpProperties: PropertiesConfig.SftpProperties = PropertiesConfig.getSftpProperties(),
) {
    private val jsch: JSch =
        JSch().apply {
            JSch.setLogger(JSchLogger())
            addIdentity(sftpProperties.privateKey, sftpProperties.keyPassword)
        }
    ...
    fun <T> channel(operation: (ChannelSftp) -> T): T {
        ...
        try {
            session =
                jsch.getSession(sftpProperties.user, sftpProperties.host, sftpProperties.port).apply {
